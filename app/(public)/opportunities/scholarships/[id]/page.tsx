...
export default function ScholarshipDetailPage() {
  ...
  return (
    <div>
      <div className="bg-gradient-to-r from-[#4c1d95] to-[#7c3aed] py-10 px-4 text-white">
        <div className="max-w-4xl mx-auto">
          <Link href="/opportunities/scholarships" className="flex items-center gap-1 text-purple-300 text-sm mb-4 hover:text-white"><ChevronLeft className="w-4 h-4" /> Back to Scholarships</Link>
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <span className="bg-purple-400/30 text-purple-200 text-xs font-bold px-3 py-1 rounded-full">{item.funding_type}</span>
            <span className="bg-white/20 text-white text-xs px-3 py-1 rounded-full">{item.study_level}</span>
          ...
          <p className="text-purple-200">{item.provider}</p>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="text-center"><MapPin className="w-5 h-5 text-purple-600 mx-auto mb-1" /><p className="text-xs text-gray-500">Country</p><p className="text-sm font-semibold">{item.country}</p></div>
              <div className="text-center"><GraduationCap className="w-5 h-5 text-purple-600 mx-auto mb-1" /><p className="text-xs text-gray-500">Study Level</p><p className="text-sm font-semibold">{item.study_level}</p></div>
              <div className="text-center"><Calendar className="w-5 h-5 text-purple-600 mx-auto mb-1" /><p className="text-xs text-gray-500">Deadline</p><p className="text-sm font-semibold">{formatDate(item.deadline)}</p></div>
            </div>
            ...
            {item.eligibility && <div className="bg-white rounded-2xl border border-gray-100 p-6"><h2 className="font-bold text-gray-900 text-lg mb-3">Eligibility Requirements</h2><div className="space-y-2">{item.eligibility.split("\n").filter(Boolean).map((e, i) => <div key={i} className="flex items-start gap-2 text-sm text-gray-600"><span className="w-1.5 h-1.5 rounded-full bg-purple-600 flex-shrink-0 mt-2" />{e}</div>)}</div></div>}
            {item.required_documents && <div className="bg-white rounded-2xl border border-gray-100 p-6"><h2 className="font-bold text-gray-900 text-lg mb-3">Required Documents</h2><div className="space-y-2">{item.required_documents.split("\n").filter(Boolean).map((d, i) => <div key={i} className="flex items-start gap-2 text-sm text-gray-600"><FileText className="w-4 h-4 text-purple-500 flex-shrink-0 mt-0.5" />{d}</div>)}</div></div>}
          </div>

          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 p-5 sticky top-20">
              {daysLeft > 0 ? (
                <div className={cn("text-center mb-4 p-3 rounded-xl", daysLeft <= 7 ? "bg-red-50" : "bg-purple-50")}>
                  <p className={cn("text-2xl font-extrabold", daysLeft <= 7 ? "text-red-700" : "text-purple-700")}>{daysLeft} days left</p>
                  <p className={cn("text-xs", daysLeft <= 7 ? "text-red-500" : "text-purple-500")}>Deadline: {formatDate(item.deadline)}</p>
                </div>
              ) : (
                <div className="text-center mb-4 p-3 bg-red-50 rounded-xl"><p className="text-red-700 font-bold">Applications Closed</p></div>
              )}
              <a href={item.application_link} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-3 bg-purple-700 text-white font-bold rounded-xl hover:bg-purple-900 mb-3">
                Apply Now <ExternalLink className="w-4 h-4" />
              </a>
              <div className="flex gap-2">
                <button onClick={() => { setSaved(!saved); toast.success(saved ? "Removed" : "Saved!"); }} className={cn("flex-1 flex items-center justify-center gap-1.5 py-2.5 border rounded-xl text-sm font-medium", saved ? "bg-purple-50 border-purple-600 text-purple-700" : "border-gray-200 text-gray-600")}>
